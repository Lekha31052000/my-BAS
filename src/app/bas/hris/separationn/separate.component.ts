...
export class SeparateComponent implements OnInit, OnDestroy {
  ...
  public pageData = new PaginationClass({ pageSize: 5, sort: { createddate: -1 } });
  ...
  listenToEvents() {
    this.searchSubscription = this.usersService.getSubscription('search').subscribe(res => {
      ...
      if (res && res.hasOwnProperty('separate') && res.hasOwnProperty('searchText')) {
